...
export function ResultPanel({ result, answeredCount }: Props) {
  return (
    ...
    >
      ...
      <div className="mt-6">
        {result ? <ResultMemo result={result} /> : <ResultPlaceholder answeredCount={answeredCount} />}
      </div>
    </section>
  );
}
...
function ResultPlaceholder({ answeredCount }: { answeredCount: number }) {
  return (
    <div className="cys-card-elevated px-6 py-10 sm:px-12 sm:py-14">
      <span className="cys-pill inline-flex h-7 items-center px-3 text-xs">
        Awaiting answers
      </span>
      <p className="cys-text mt-5 max-w-2xl text-lg leading-7 sm:text-xl sm:leading-8">
...
function ResultMemo({ result }: { result: AssessmentResult }) {
  return (
    <article className="cys-card-elevated px-6 py-8 sm:px-12 sm:py-12">
      ...
      <h3 className="cys-text mt-5 text-[1.75rem] font-semibold leading-tight sm:text-[2.25rem]">
        {result.label}
      </h3>
      ...
      <p className="cys-text-subtle mt-5 max-w-3xl text-sm leading-6">
        {RESULT_DISCLAIMER}
      </p>

      <hr className="cys-divider mt-8" />
      ...
      <div className="mt-8 grid gap-6 sm:grid-cols-2">
        ...
        <div className="flex flex-col gap-4">
          <div className="cys-card-muted px-5 py-5">
